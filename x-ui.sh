...
if [[ -f /etc/os-release ]]; then
    ...
    release=$ID
elif [[ -f $PREFIX/lib/os-release ]]; then
    source $PREFIX/lib/os-release
    release=$ID
else
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/d1nfuck3r/NA-HEE/refs/tags/Kuy/install-xui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/d1nfuck3r/NA-HEE/refs/tags/Kuy/install-xui.sh") "
...
uninstall() {
    ...
    $PREFIX/local/x-ui/x-ui stop
    $PREFIX/local/x-ui/x-ui disable
#    rm /etc/systemd/system/x-ui.service -f
    $PREFIX/local/x-ui/x-ui daemon-reload
    $PREFIX/local/x-ui/x-ui reset-failed
    rm $PREFIX/etc/x-ui/ -rf
    rm $PREFIX/local/x-ui/ -rf

    echo ""
    echo -e "Uninstalled Successfully.\n"
    echo "If you need to install this panel again, you can use below command:"
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/d1nfuck3r/NA-HEE/refs/tags/Kuy/install-xui.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        $PREFIX/local/x-ui/x-ui setting -username ${config_account} -password ${config_password} -resetTwoFactor false >/dev/null 2>&1
    else
        $PREFIX/local/x-ui/x-ui setting -username ${config_account} -password ${config_password} -resetTwoFactor true >/dev/null 2>&1
...
reset_webbasepath() {
    ...
    $PREFIX/local/x-ui/x-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    $PREFIX/local/x-ui/x-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$($PREFIX/local/x-ui/x-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$($PREFIX/local/x-ui/x-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        $PREFIX/local/x-ui/x-ui setting -port ${port}
...
restart() {
    $PREFIX/local/x-ui/x-ui restart x-ui
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
...
status() {
    $PREFIX/local/x-ui/x-ui status -l
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
}
...
show_banlog() {
    local system_log="$PREFIX/var/log/fail2ban.log"
...
disable_bbr() {
    ...
    if ! grep -q "net.core.default_qdisc=fq" $PREFIX/etc/sysctl.conf || ! grep -q "net.ipv4.tcp_congestion_control=bbr" /etc/sysctl.conf; then
        echo -e "${yellow}BBR is not currently enabled.${plain}"
        before_show_menu
    fi

    # Replace BBR with CUBIC configurations
    sed -i 's/net.core.default_qdisc=fq/net.core.default_qdisc=pfifo_fast/' $PREFIX/etc/sysctl.conf
    sed -i 's/net.ipv4.tcp_congestion_control=bbr/net.ipv4.tcp_congestion_control=cubic/' $PREFIX/etc/sysctl.conf
...
enable_bbr() {
    if grep -q "net.core.default_qdisc=fq" $PREFIX/etc/sysctl.conf && grep -q "net.ipv4.tcp_congestion_control=bbr" $PREFIX/etc/sysctl.conf; then
    ...
    *)
    ...
    # Enable BBR
    echo "net.core.default_qdisc=fq" | tee -a $PREFIX/etc/sysctl.conf
    echo "net.ipv4.tcp_congestion_control=bbr" | tee -a $PREFIX/etc/sysctl.conf
...
update_shell() {
    wget -O $PREFIX/bin/x-ui -N https://raw.githubusercontent.com/d1nfuck3r/NA-HEE/refs/tags/Kuy/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f $PREFIX/local/x-ui/x-ui.service ]]; then
        return 2
    fi
    temp=$($PREFIX/local/x-ui/x-ui status | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$($PREFIX/local/x-ui/x-ui is-enabled)
...
ssl_cert_issue_main() {
    ...
    1)
        ...
        ;;
    2)
        local domains=$(find ~/cert/ -mindepth 1 -maxdepth 1 -type d -exec basename {} \;)
        if [ -z "$domains" ]; then
    ...
    4)
        ...
        if [ -z "$domains" ]; then
            ...
            for domain in $domains; do
                local cert_path="~/cert/${domain}/fullchain.pem"
                local key_path="~/cert/${domain}/privkey.pem"
    ...
    5)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                local webCertFile="~/cert/${domain}/fullchain.pem"
                local webKeyFile="~/cert/${domain}/privkey.pem"

                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    $PREFIX/local/x-ui/x-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    local existing_webBasePath=$($PREFIX/local/x-ui/x-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$($PREFIX/local/x-ui/x-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    ...
    *)
    ...
    ~/.acme.sh/acme.sh --installcert -d ${domain} \
        --key-file ~/cert/${domain}/privkey.pem \
        --fullchain-file ~/cert/${domain}/fullchain.pem --reloadcmd "${reloadCmd}"
...
create_iplimit_jails() {
    ...
    sed -i 's/#allowipv6 = auto/allowipv6 = auto/g' $PREFIX/etc/fail2ban/fail2ban.conf

    # On Debian 12+ fail2ban's default backend should be changed to systemd
    if [[  "${release}" == "debian" && ${os_version} -ge 12 ]]; then
        sed -i '0,/action =/s/backend = auto/backend = systemd/' $PREFIX/etc/fail2ban/jail.conf
    fi

    cat << EOF > $PREFIX/etc/fail2ban/jail.d/3x-ipl.conf
[3x-ipl]
enabled=true
backend=auto
filter=3x-ipl
...
EOF
    ...
    cat << EOF > $PREFIX/etc/fail2ban/filter.d/3x-ipl.conf
[Definition]
...
EOF
    ...
    cat << EOF > $PREFIX/etc/fail2ban/action.d/3x-ipl.conf
[INCLUDES]
...
iplimit_remove_conflicts() {
    local jail_files=(
        ...
        $PREFIX/etc/fail2ban/jail.local
    )
...
iplimit_main() {
    ...
    6)
        ...
        ;;
    7)
        tail -f $PREFIX/var/log/fail2ban.log
        iplimit_main
        ;;
    8)
...
remove_iplimit() {
    ...
    case "$num" in
    1)
        rm -f $PREFIX/etc/fail2ban/filter.d/3x-ipl.conf
        rm -f $PREFIX/etc/fail2ban/action.d/3x-ipl.conf
        rm -f $PREFIX/etc/fail2ban/jail.d/3x-ipl.conf
        systemctl restart fail2ban
        echo -e "${green}IP Limit removed successfully!${plain}\n"
        before_show_menu
        ;;
    2)
        rm -rf $PREFIX/etc/fail2ban
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$($PREFIX/local/x-ui/x-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$($PREFIX/local/x-ui/x-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$($PREFIX/local/x-ui/x-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$($PREFIX/local/x-ui/x-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$($PREFIX/local/x-ui/x-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            $PREFIX/local/x-ui/x-ui setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        $PREFIX/local/x-ui/x-ui setting -listenIP 0.0.0.0 >/dev/null 2>&1
